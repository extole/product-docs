...
description: "Validate Conversion Audit outcomes and earned rewards against graph-derived earning steps."
noindex: true
---
Validate processed conversion/outcome evidence and earned rewards for one live program. V10 flow-builder is the primary target; V8/legacy campaigns run these reports best-effort with enabled purchase-incentivize steps derived from V8 controller configuration (see [Overview](//runbooks/functional-review/overview)).
...
<Warning>
**Do **not** use reward event names. Use the business-event step names that pass the V8 / V10 enabled-reward test from the campaign graph.**
...
<Warning>
**Fail-closed — including a step that fails the V8 / V10 enabled-reward test is invalid execution.**
