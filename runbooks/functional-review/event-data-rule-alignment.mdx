...
description: "Compare Input Records event.data samples against trigger and quality rule conditions from the campaign graph."
noindex: true
---
...
3. Input Records used graph-derived `event_names` from `triggerEventNames` / `eventNames` — not business-event component names. On a V8/legacy campaign, use the legacy fallback names from [Input / Runtime Event Validation](//runbooks/functional-review/input-runtime-event-validation).
4. A graph-derived expectation manifest exists (see [Input / Runtime Event Validation](//runbooks/functional-review/input-runtime-event-validation)). On a V8/legacy campaign, the manifest is built from controller/step configuration and its rules are labeled configuration-derived candidates.
...
<Warning>
****Config only.** This check is derived from the campaign graph/configuration alone. Inbound events do not label which field is the unique key. Do **not** infer or validate the key from Input Records or inbound payloads.**
</Warning>
...
- Run **after** Input Records is downloaded ([Input / Runtime Event Validation](//runbooks/functional-review/input-runtime-event-validation)); **before** [Conversion & Reward Validation](//runbooks/functional-review/conversion-reward-validation) conclusions.
