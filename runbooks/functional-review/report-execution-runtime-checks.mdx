...
description: "Canonical execution rules for a Functional Review — queue, hard stop, expectation manifest, completion gate, and output format."
noindex: true
---
Use this guide for the **report-execution and runtime-check** portion of a Functional Review on **one live referral or rewards program**. V10 flow-builder is the primary target; V8/legacy campaigns run the same queue best-effort under the legacy rules in [Overview](//runbooks/functional-review/overview).
...
Use this after [Overview](//runbooks/functional-review/overview) has identified:
...
<Warning>
****`V10 confirmed: No` does not block execution.** When the campaign is V8/legacy, still build the full queue, submit every report, and complete the review — with the legacy disclaimer and configuration-confidence limits from [Overview](//runbooks/functional-review/overview). No report in this queue requires V10.**
</Warning>
...
<Warning>
****Never reuse Count’s id for Triggered Steps.** Submitting `t1owor6ia18bia3ur7zg` with display name “Input Events with Triggered Steps” (or only changing `period`) is **Invalid execution** — resubmit with `INPUT_EVENTS_WITH_TRIGGERED_STEPS` before analyzing event→step mapping.**
</Warning>
...
3. Re-read the specific report section and verify section-level conditions:
   ...
   - submitted `report_type` matches the documented type for that queue item (Triggered Steps must be `INPUT_EVENTS_WITH_TRIGGERED_STEPS`, never Count’s `t1owor6ia18bia3ur7zg`; Conversion Audit `step_names` must pass the V8 / V10 enabled-reward test in [Conversion & Reward Validation](//runbooks/functional-review/conversion-reward-validation) — V10: enabled `earn-reward` or `qualify` only, not bare `incentivize`; V8: enabled purchase `incentivize` only)
...
Also check partner-key **consistency** across the campaign (more than one distinct configured key) in [Event Data Rule Alignment](//runbooks/functional-review/event-data-rule-alignment) — Partner event key consistency (configuration only).
...
- Conversion Audit attempted with `step_names` that pass the V8 / V10 test in [Conversion & Reward Validation](//runbooks/functional-review/conversion-reward-validation) (V10: enabled `earn-reward` or `qualify` only — bare `incentivize` is not enough; V8: enabled purchase `incentivize` only; disabled qualifying reward actions omitted from `step_names` and listed in the review output)
...
- **Legacy disclaimer** — required when the campaign is V8/legacy (see [Overview](//runbooks/functional-review/overview))
...
- [Input / Runtime Event Validation](//runbooks/functional-review/input-runtime-event-validation)
- [Event Data Rule Alignment](//runbooks/functional-review/event-data-rule-alignment)
- [Conversion & Reward Validation](//runbooks/functional-review/conversion-reward-validation)
- [Email / Webhook / Side-Effect Validation](//runbooks/functional-review/email-webhook-side-effect-validation)
- [Terms, Rewards, and Configuration Alignment](//runbooks/functional-review/terms-rewards-and-configuration-alignment)
