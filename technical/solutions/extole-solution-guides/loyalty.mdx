...
| `partner_user_id` **required** | Your unique identifier for this person, such as an account ID or member ID. This is the key their balance is held against. |
...
| `partner_conversion_id` **required** | Your order number, uniquely identifying this transaction. Used for deduplication and customer support. |
...
Send a request for each earning action that happens outside your site. Extole deduplicates events by order identifier, so the same purchase can safely arrive by both tag and API.
...
This number sets the cost of your program and how the offer reads to a member. Choose it before launch and treat changes as a policy change. See [Choosing Your Point-to-Reward Ratio](/guides/strategy-and-best-practices/campaign-optimization/choosing-your-point-to-reward-ratio).
...
Replace `client` with your program domain, which you can find in the [Tech Center](https://my.extole.com/tech-center).

```curl
curl --get "https://client.extole.io/zone/bankable_rewards" \
...
Two operations remove value a member has already earned. Both are audited and both accept a reason stored with the record.
...
Use `expire` for a reward that has reached the end of its life under your program terms, and `revoke` for one that should not stand — issued against an order later returned, or against activity found to be illegitimate.
...
Surface earning and redemption in your app with the [Mobile SDKs](/technical/platform-integrations/mobile-sdks), or render a balance in your own interface with the [Headless and Mobile API](/technical/platform-integrations/rest-apis/mobile-api).
