...
<Warning>
  Send the hint **with** the program label, not instead of it. The hint narrows the candidates to
  the campaign you named, and journey entry then still needs the label to start the journey. A
  request carrying the hint and no label matches nothing and comes back empty, so the share
  experience does not open at all.
</Warning>
