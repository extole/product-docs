...
| [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) | Business events, input event rules, event data, and the test that proves each inbound event appears in Event Streams | Partner to Extole |
...
**Partner to Extole.** Send a synchronous event to `POST /v6/events` with the current program label in `data.labels`, confirm the Input event in Event Streams, then read the step it produced:
