...
### When a Test Event Produces No Step

An arriving event stops at one of five stages: arrival, identity, matching, evaluation, or reporting. Each tool in My Extole sees only some of them, so pick the one that can see the stage you're checking:

- **Event Streams** show every event live, from the moment you click **Start Streaming** — start the stream before you send the test. An `INPUT` event followed by a `STEP` event arrived and matched a campaign; an `INPUT` event with nothing after it matched nothing. The event-stream view this guide builds is an Event Stream filtered to the integration's app type.
- **Event Live View** looks back in time, but lists only events a campaign acted on. An input event that matched no campaign never appears in it.
- **A person's steps**, under **Support** > **Person Search** > **Show Steps**, show one person's full history. A step that appears under **All Containers** but not **Production Only** went to a test container, and production reports won't count it.
- **The Debugging reports** check the whole feed over any time range. **Input event names with triggered steps** finds a partner event name that arrives in volume and creates no step — usually a rule listening for a name the partner doesn't send.

For a partner integration, check the vocabulary first: the partner's own event name belongs on the `input_event` rule, and the canonical Extole name belongs on the step. [Debug an Inbound Integration](/technical/troubleshooting/debug-an-inbound-integration) walks through each tool, a seven-check sequence that finds the stage where an event stopped, and what to send Extole Support.
