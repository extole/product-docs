...
**Partner to Extole.** Send a synchronous event to `POST /v6/events` with the current program label in `data.labels`, confirm the `INPUT` event and the `STEP` event it produced in the integration's event-stream tab, then read the step through the API:
