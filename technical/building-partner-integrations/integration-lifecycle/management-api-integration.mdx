...
**Partner to Extole.** Send a synchronous event to `POST /v6/events` with the current program label in `data.labels`, confirm the `INPUT` event and its resulting `STEP` event in the integration's **Event Stream** tab, then read the step it produced:
