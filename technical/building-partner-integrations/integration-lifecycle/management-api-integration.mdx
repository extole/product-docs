...
| [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) | Business events, input event rules, event data, and the test that proves each partner event reaches a business event | Partner to Extole |
...
**Partner to Extole.** Send a synchronous event to `POST /v6/events` with the current program label in `data.labels`, confirm the Input and correlated STEP in the integration's **Event Stream** tab, then read the event it produced:
