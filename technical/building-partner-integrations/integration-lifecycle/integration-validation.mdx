...
For each inbound wire name, record the `event_id` returned by the send and confirm the Input event and the correlated STEP are visible on the integration Event Stream tab before you call the integration working. A FAIL starts a correction loop: use the matching logs and mapped data to fix the integration, republish when required, and send a fresh unique event until the chain passes. Connecting the integration to a program is the next section; it does not replace this gate. [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) covers how to send the event, how to query by `event_id` and `root_event_id`, and when to call PASS, FAIL, or INCONCLUSIVE.
...
[Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) names the fields each event captures. A complete integration of this shape carries the model component, three business events, one `input_event` rule per event, one data component per captured field, and the three baseline views. It is Partner to Extole only: no reward-supplier socket, reward webhook, or client key.
