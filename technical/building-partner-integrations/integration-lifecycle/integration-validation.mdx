...
For each inbound wire name, use the `event_id` returned by the send to find the Input and correlated STEP in the integration's **Event Stream** tab. Confirm the STEP has the canonical name, expected person, mapped data, and attributed campaign context. Use the matching logs to correct any difference, then republish when required and send a fresh unique event. Connecting the integration to a program does not replace this test. [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) covers the complete sequence.
