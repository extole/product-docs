...
For each partner wire name, use the `event_id` returned by the send to find the `INPUT` event and its resulting `STEP` event in the integration's **Event Stream** tab. Confirm the `STEP` event has the canonical name, expected person, mapped data, and attributed campaign context. Connecting the integration to a program does not replace this test. [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) covers the complete sequence.
