...
description: "Gate Extole to Partner resources, validate the finished shape, publish the validated version, and connect the integration to a program."
---
...
Validation is the last gate before an integration goes live: decide whether it should own Extole to Partner resources, check the built shape against the model, then publish the version you checked. Connecting its business events to a marketing program is a separate, approved step.

## Gate Extole to Partner Resources

This applies to a Partner to Extole build someone proposes to extend in the other direction; the webhooks and credential a maintained Extole to Partner install ships with are part of its finished shape, not gated here.

Create Extole to Partner resources only when all of these hold:

- The requested scope includes a defined Extole to Partner use case.
...
When Extole to Partner scope is later removed, archive resources in dependency order:
...
- Where the build must appear as an installed integration, its owner and tags match what [Build a Partner Integration with the Management API](/technical/building-partner-integrations/integration-lifecycle/management-api-integration) describes; read the tags back.
...
- The socket holds the three baseline views, a **Test Event** `config-view-v10.0` child that lists the parent integration's `testEventPayload` JSON setting on a Partner to Extole build, and the `reward-suppliers` configuration view on a reward fulfillment build.
...
- Every column named in `reportColumnsMapping` is one the report runner's `mappings` expression produces, and the event stream is running with an empty `filters` collection.

### Display Settings

Read all eight display settings from `/v1/components/built` and check each against the value convention in [Create the Integration Campaign and Component Model](/technical/building-partner-integrations/integration-lifecycle/integration-component-model). Present is not usable: `short.description` and `about` describe this partner rather than integrations generally, `documentation.url` points at the partner-facing page rather than this build guide, and `categories` spells a string other integrations already use.
...
For each partner wire name, send a test event and use its `event_id` to find the `INPUT` event and the `STEP` event it produced in the integration's event-stream tab. Confirm the `STEP` event has the canonical name, the expected person, the mapped data, and an attributed campaign context. Connecting the integration to a program does not replace this test. [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) covers the complete sequence.
...
1. Publish the integration campaign, which makes the event endpoint accept events.
...
[Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) names the fields each event captures. A complete integration of this shape carries the model component, three business events, one `input_event` rule per event, one data component per captured field, the three baseline views, a `testEventPayload` JSON setting, and the **Test Event** configuration view that displays it. It is Partner to Extole only: no reward-supplier socket, reward webhook, or client key.
