...
When the payload produces no step, [Debug an Inbound Integration](/technical/troubleshooting/debug-an-inbound-integration) finds the stage where it stopped: an Event Stream shows whether it arrived and under which name, **Input event names with triggered steps** shows whether any rule matched it, and the person's steps show which container the step landed in.
...
- [Debug an Inbound Integration](/technical/troubleshooting/debug-an-inbound-integration)
