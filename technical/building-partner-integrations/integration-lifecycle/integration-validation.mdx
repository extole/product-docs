...
For each inbound wire name, record a test event identifier and confirm the Input event is visible in Event Streams before you call the integration working. Connecting the integration to a program is the next section; it does not replace this gate. [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) covers how to send the event and which stream to watch.
