...
description: "Build the report-runner and event-stream views every integration needs, plus the test-event view for Partner to Extole integrations."
---
...
Every integration needs a **report-runner view** charting what it processed and an **event-stream view** carrying its live event feed. Partner to Extole integrations also need a test-event view. [Integration Categories](/technical/building-partner-integrations/integration-types/integration-categories) covers each category's views.
...
### Add the Test Event View to a Partner to Extole Integration
...
The view builds an editable payload from the integration's business events, enabled input rules, and program label. Add one test-event view per integration. [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events#how-to-test) covers its generated fields, including referral data and analytics isolation.
