...
Every integration needs a **report-runner view** charting what it processed and an **event-stream view** carrying its live event feed. This page builds both. A Partner to Extole integration also gets a **Test Event** tab, which [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events) adds once its business events and input rules exist.
...
Create the stream after the republish above, and add no filters. A stream records an event only when every filter matches, and the events one input event causes — the `STEP` business event, rewards, and deliveries — can carry a different application type or event type from the `INPUT` event. A filter on either hides part of that chain, which is exactly what you need to see when you test the integration.
...
An unfiltered stream records every event in the account while it runs, so narrow what you read instead of what it records. After a test send, search the stream for the returned `event_id`, then for the events carrying the same `root_event_id`. [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events#how-to-test) walks through the search.
