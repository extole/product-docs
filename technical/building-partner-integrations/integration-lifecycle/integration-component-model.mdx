...
Declare a type on every socket and attach only components that satisfy it. `view-v10.0` is the parent of the standard view types, so one filter accepts the configuration, activity, and event-stream views from [Add the Activity and Event Views](/technical/building-partner-integrations/integration-lifecycle/integration-activity-views). A Partner to Extole integration also adds a `testEventPayload` JSON setting and displays it through a `config-view-v10.0` child titled **Test Event**. Add both after configuring the business events and input rules in [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events).
...
| A component's `name` changed after an edit that only set its label | `display_name` sent without `name` on the update, which re-derives `name` from the label | Send `name` alongside `display_name` on every update |
