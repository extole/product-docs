...
An Extole to Partner integration forwards Extole program activity to a partner platform. Install and reshape a maintained source when it contains the required behavior. When a partner source contains only display metadata, construct the integration-owned controllers and webhook actions that send the activity.
