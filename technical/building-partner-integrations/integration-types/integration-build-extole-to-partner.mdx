...
| `CAMPAIGN_ID`, `CAMPAIGN_VERSION` | The integration campaign and its current version. Every change creates a new version, so read the version again before each versioned call. |
...
List the duplicatable sources whose name matches the partner exactly, accepting every `integration-v10.*` revision — [Install the Maintained Source](#install-the-maintained-source) shows the query — then inspect the integration component and its children.
...
Choose each controller's trigger by the kind of activity it forwards:
...
| A fulfilled reward | A reward-event trigger with `reward_states` set to `FULFILLED`, created through `.../controllers/{controller_id}/triggers/reward-events`. |
...
Forwarding a fulfilled reward to a messaging or analytics partner is not the same as a partner that supplies the reward. A gift card, prepaid card, points, or payout provider uses [Build a Reward Fulfillment Integration](/technical/building-partner-integrations/integration-types/integration-build-reward-fulfillment).
...
Record the returned controller identifier as `CONTROLLER_ID`.
...
For share-link creation, use the same endpoint with `event_type` set to `SHAREABLE` and read the link from `context.causeEvent.shareable.link`.

Create every controller and trigger while the path is disabled. Then publish and create the component-scoped webhook, as described in [Publish, Then Attach Component-Scoped Webhooks](#publish-then-attach-component-scoped-webhooks), and attach it to the controller through [Attach the Webhook Action](#attach-the-webhook-action). Take field names and identity rules from the partner page.
...
Record the returned webhook identifier as `WEBHOOK_ID`.
...
curl -s -X POST -H "Authorization: Bearer ${TOKEN}" \
  ...
  -d '{
    ...
    "data": {
      "person_identifier": "spel@runtime:context.person.email",
      "activity_name": "extole_converted",
      "activity_time": "spel@runtime:context.causeEvent.eventTime"
    }
  }' \
