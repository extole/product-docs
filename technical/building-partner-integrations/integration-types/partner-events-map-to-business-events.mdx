...
For a prehandler-backed integration, set `event_name` to the path segment the prehandler matches with `EVENT_NAME_MATCH` and put the partner's raw webhook body in `data`. The platform resolves both Events API forms to the same event name and data, so the prehandler sees what it would have seen had the partner posted the body to `POST /v6/events/{event_name}` itself.
