...
After configuring the business events and input rules, add a `testEventPayload` JSON setting to the integration component. For an Extole-shaped event, author the payload with the partner wire event name, representative identity and event data, the current program label, and `data.container` set to `test`. For a prehandler-backed integration, preserve the partner's raw webhook body without adding `event_name`; the Test Event control supplies the integration name and a top-level test container when it sends the payload. Include a distinct `advocate.email` when the event produces a `FRIEND` journey.
...
- For an Extole-shaped event, keep `data.container` set to `test`. To test another wire event, change `event_name` and the event-specific data fields.
- For a prehandler-backed integration, keep the partner's raw webhook body without adding `event_name`. The control sends it through the named Events API endpoint and adds a top-level `container` with value `test`.
