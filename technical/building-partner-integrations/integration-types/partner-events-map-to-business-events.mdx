...
The three baseline views are the same for every category — see [Integration Categories](/technical/building-partner-integrations/integration-types/integration-categories). Add the test-event view so My Extole can build an editable payload from the configured wire events, and build the sender against [Send Events from the Partner Platform](/technical/building-partner-integrations/integration-types/partner-events-send-from-platform).
...
## Add the Test Event View

After configuring the business events and input rules, add one test-event view to the integration component's `views` socket. The `test-event-view-v10.0` type makes My Extole render the **Test Event** tab.
...
The view builds its editable payload from the integration's business events, enabled input rules, and program label.
