...
Open the integration's **Test Event** tab, edit the payload with representative values, and select **Send Event**. Keep `data.container` set to `test`; it excludes the event from production dashboard analytics and reports without disabling event processing or rewards. To test another wire event, change `event_name` and the event-specific data fields. For a `FRIEND` journey, keep `advocate.email` distinct from the friend's `email`.

You can also send the same payload through `POST /v6/events` with an event-submission token, not the token that created the campaign. Put the current program label in `data` to target this campaign; it is a testing convenience and does not belong in the partner's production payload. See [Exclude Test Data from Analytics](/technical/operational-tasks/program-testing/exclude-test-data-from-analytics).
