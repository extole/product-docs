...
description: "Add business events, input event rules, and event data to a partner integration that receives partner events, then verify what arrives."
---
...
The three baseline views are the same for every category — see [Integration Categories](/technical/building-partner-integrations/integration-types/integration-categories). Add a test event payload setting and display it through a configuration view so My Extole can send an editable test payload, and build the sender against [Send Events from the Partner Platform](/technical/building-partner-integrations/integration-types/partner-events-send-from-platform).
...
After configuring the business events and input rules, add a `testEventPayload` JSON setting to the integration component. The setting holds a complete `POST /v6/events` request body, which My Extole sends exactly as written: author `event_name`, an optional `event_time`, and a `data` object carrying representative identity and event data, the current program label, and `container` set to `test`. Include a distinct `advocate.email` when the event produces a `FRIEND` journey.

For a prehandler-backed integration, set `event_name` to the path segment the prehandler matches with `EVENT_NAME_MATCH` and put the partner's raw webhook body in `data`. The platform resolves both Events API forms to the same event name and data, so the prehandler sees what it would have seen had the partner posted the body to `POST /v6/events/{event_name}` itself. This holds only while every condition on the prehandler is one a My Extole send can satisfy: a prehandler that also requires a partner header never fires from this control, so give it the event name the prehandler produces and the flattened data its actions would have written — see [Normalize Partner Events with a Prehandler](/technical/building-partner-integrations/integration-types/partner-events-prehandlers).
...
Add a `config-view-v10.0` child to the integration component's `views` socket. Its `settingsToDisplay` names the parent integration setting, which makes My Extole render the payload and **Send Event** button in the **Test Event** tab.
...
Open the integration's **Test Event** tab, edit the payload with representative values, and select **Send Event**.

The control sends the payload exactly as authored and adds nothing, so every field the event needs must be in the setting.

- Keep `data.container` set to `test`. To test another wire event, change `event_name` and the event-specific data fields.
...
The test container excludes the event from production dashboard analytics and reports without disabling event processing or rewards.

An external command-line sender can submit the Extole-shaped payload through `POST /v6/events` with an event-submission token, not the token that created the campaign. Put the current program label in `data` to target this campaign; it is a testing convenience and does not belong in the partner's production payload. My Extole's **Send Event** control and chat tools use your authenticated client context. Never paste an access token or other credential into My Extole chat. See [Exclude Test Data from Analytics](/technical/operational-tasks/program-testing/exclude-test-data-from-analytics).
...
A `2xx` means Extole accepted the event, not that it matched a business event. Open the integration's **Event Stream** tab and confirm the stream is running with an empty `filters` collection before sending. [Add the Activity and Event Views](/technical/building-custom-integrations/integration-lifecycle/integration-activity-views) shows how to create this stream.

Capture `event_id` from the synchronous response, then search with [Event Stream Query Language](/technical/platform-integrations/extensions/event-streams-overview/event-stream-query-language):

1. Search `event_id=` followed by the returned identifier to find the `INPUT` event.
2. Search `root_event_id=` followed by the same identifier to find downstream events.

Open the JSON for the `INPUT` event and its descendants:

- The `INPUT` event's `name` is the wire event you submitted. Its `raw_data` contains the received payload, including `container: test` and, for a friend journey, `advocate.email`.
- A descendant with `type` `STEP` and the canonical `name` is the mapped business event. Its `data` contains the mapped values, and its `person_profile` should match the `INPUT` event.
- `campaign_context.quality` of `HIGH` or `LOW` means a campaign selected the event. `NONE`, or a missing `campaign_context`, means the event was recorded without attribution.
- `duplicate` is `true` when the unique partner event identifier has already been processed.
- When no campaign selects the event, `step_matching_campaign_contexts` shows each candidate. Use its `trigger_results` and `log_messages`, together with the `INPUT` event's `log_messages`, to find the rule that did not match.

Correct any mapping or rule that differs from the expected result, republish when required, and send a fresh unique event. Restore a stopped or filtered stream before testing again.
