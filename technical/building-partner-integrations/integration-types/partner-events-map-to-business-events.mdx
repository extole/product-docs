...
After configuring the business events and input rules, add a `testEventPayload` JSON setting to the integration component. Author the payload with a partner wire event name, representative identity and event data, the current program label, and `container: "test"`. Include a distinct `advocate.email` when the event produces a `FRIEND` journey.
