...
The membership variant of [Sending Partner Events to Extole](/technical/building-partner-integrations/integration-types/partner-events-to-extole).
...
This travels Partner to Extole like the rest of this group, but the partner sends **who is eligible** rather than **what happened**. It carries no business events, so it needs no sender, no prehandler, and no event mapping — and it is the one shape in this direction that needs a credential.
...
If the partner has a maintained source, install it first and add audience membership to the installed campaign. For other integration paths, see [Build a Partner Integration with the Management API](/technical/building-partner-integrations/integration-lifecycle/management-api-integration).
...
In addition to [Validate and Publish an Integration](/technical/building-partner-integrations/integration-lifecycle/integration-validation):
