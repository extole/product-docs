...
```bash
curl --request POST "$EXTOLE_API_HOST/v6/reward-suppliers" \
  --header "Authorization: Bearer $MANAGEMENT_API_ACCESS_TOKEN" \
  --header "Content-Type: application/json" \
  --data '{
...
A partner fulfilling its own products creates a `CUSTOM_REWARD` supplier, where `type` is the custom reward kind rather than the component type the template carries.
...
Add no filters to the event stream, so the `INPUT`, `STEP`, `REWARD`, `SEND_REWARD`, and delivery events of one reward stay visible as a single chain. The stream then records every event in the account while it runs, so narrow an investigation when you read it, using the identifiers returned by the originating event.
