...
<Warning>
A valid submission returns `200` with a JSON body carrying both `event_id` and `person_id`. Keep the `event_id`: it is how you find the event, and every event it caused, in an event stream. An empty `204 No Content` means the request is not reaching the Events API — check the host.
</Warning>
...
Confirm the event carries the canonical event name, not the platform's; the transaction identifier and value match the source record; person keys resolve; and resending the same source identifier produces a duplicate outcome, not a second conversion. Repeat for every event. [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events#how-to-test) shows how to use `event_id` to follow the complete processing chain in Event Streams.
...
2. The platform event name matches a name on the integration's `input_event` trigger rule. The comparison ignores case, so casing is not the cause.
