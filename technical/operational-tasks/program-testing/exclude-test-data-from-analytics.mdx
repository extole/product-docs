...
The dashboard offers the containers `all`, `production`, `test`, `pending-review`, and `bot`. An event routed to any other container name is stored, but it appears in none of the dashboard views, so a figure that looks too low is worth decomposing per container before you conclude that events were lost.
