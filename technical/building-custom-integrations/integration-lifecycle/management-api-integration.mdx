...
- **Carry all three baseline views** — configuration, report-runner, and event-stream. The category decides what the report counts, while the event stream keeps a complete unfiltered processing chain. Reward fulfillment adds a fourth, a `reward-suppliers` configuration view.
