...
| [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) | Business events, input event rules, and event data | Inbound |
...
**Inbound.** Send a synchronous event to `POST /v6/events` with the current program label in `data.labels`, then read the step it produced:
