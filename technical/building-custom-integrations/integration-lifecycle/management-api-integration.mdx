...
A request that adds inbound scope to a maintained outbound partner uses both paths: install the library source first, then add the inbound half to the installed campaign — business events for a partner that reports activity, or an audience and credential for one that sends eligibility. Check the finished integration against every category the request matched.
