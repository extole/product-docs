...
| Inbound audience membership | The platform decides who is eligible and sends that membership to Extole, rather than reporting events. | Add an audience the integration owns, a credential the partner authenticates with, and setup instructions naming the membership endpoints. Follow [Receive Audience Membership from a Partner](/technical/building-custom-integrations/integration-types/integration-inbound-audience-membership) below. |
...
A request that adds inbound scope to a maintained outbound partner uses both paths: install the library source first, then add the inbound half to the installed campaign — business events for a partner that reports activity, or an audience and credential for one that sends eligibility. A maintained source covers only what it was built for, so check the finished integration against every category the request matched rather than against the one the partner is filed under.
...
| [Receive Audience Membership from a Partner](/technical/building-custom-integrations/integration-types/integration-inbound-audience-membership) | The audience, credential, and setup instructions for a partner that sends eligibility rather than events. |
