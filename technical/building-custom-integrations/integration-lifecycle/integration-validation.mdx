...
- The eight required integration display settings hold usable values, not merely present ones. Read each from `/v1/components/built` and check it against the value convention in [Create the Integration Campaign and Component Model](/technical/building-custom-integrations/integration-lifecycle/integration-component-model):
...
Treat a successful publish as model validation, not end-to-end verification. Nothing above proves an event reaches a step, and two of the pieces this guide creates — a prehandler script and a data component's `valueExpression` — are stored without ever being executed. Send a real payload and read the resulting step before you call the integration working.
...
This changes a program's funnel, so it is never a silent change. Identify the campaign and the events before touching them, and record which events were replaced, which were added, and what the program's journey contains afterwards. A funnel that changed shape without a record is one nobody can reconcile against last month's numbers.
