...
- Read from `/v1/components/built`, every webhook's `enabled` resolves to the integration's own toggle rather than to a literal `false`. A webhook wired to the toggle starts when the installer enables the integration; one holding a literal never starts, and no call fails to say so. Two webhooks on the same integration resolving differently is the signature. Then account for each use case the integration promises against the webhook that carries it, because a use case whose only transport can never turn on is not built.
