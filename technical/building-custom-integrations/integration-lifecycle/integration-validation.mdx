...
- Read from `/v1/components/built`, every webhook's `enabled` resolves to the integration's own toggle rather than to a literal `false`. A webhook wired to the toggle starts when the installer enables the integration; one holding a literal `false` stays off no matter what the installer does, and nothing looks wrong until someone asks why the partner never received anything. Where two webhooks on the same integration resolve differently, that is usually why. Then check that everything the integration promises has a webhook that can actually fire.
