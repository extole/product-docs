...
Declare a type on every socket and attach only components that satisfy it. `view-v10.0` is the parent of the standard view types, so one filter accepts the configuration view below plus the activity and event-stream children from [Add the Activity and Event Views](/technical/building-custom-integrations/integration-lifecycle/integration-activity-views). An inbound integration also installs a view carrying both `view-v10.0` and `test-event-view-v10.0`, which keeps it in this socket while giving My Extole a distinct renderer contract.
