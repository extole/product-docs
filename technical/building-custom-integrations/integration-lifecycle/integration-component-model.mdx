...
For a partner with no registered component anywhere and no supplied file, the artwork is the one piece the documentation cannot give you. Get the real file and track the `logo` setting as outstanding until it arrives, rather than filling the gap with an image that merely passes validation.
...
The configuration view is one of three. Continue to [Add the Activity and Event Views](/technical/building-custom-integrations/integration-lifecycle/integration-activity-views) for the report-runner view and event-stream view every integration also carries, whichever build path brought you here.
