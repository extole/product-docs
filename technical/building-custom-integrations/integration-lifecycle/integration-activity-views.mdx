...
The view has no report runner or event stream of its own. It uses the integration's built business events, enabled input rules, and program label to prepare the payload. Add one test-event view for the integration, regardless of how many wire events its rules accept.
