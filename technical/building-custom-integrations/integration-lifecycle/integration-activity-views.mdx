...
Configure an inbound integration stream with both filters shown above. The `APPLICATION_TYPE` filter selects the partner's own activity and events submitted through the Event Dispatcher. The single `EVENT_TYPE` filter selects the complete debugging chain.
...
For `POST /v6/events` and `POST /v6/async-events`, list `Event Dispatcher`. `API` identifies a Consumer API request instead. To capture every application type, omit the `APPLICATION_TYPE` filter; the API requires at least one value when the filter is present. A sender that sets `app_type` in the event data overrides the arrival value, so include that value.

A STEP inherits the Input's `event_context`, so `Event Dispatcher` captures both. Reward, send-reward, incentivized, identified, and message events are created with an empty `event_context` and pass the `APPLICATION_TYPE` filter. The `EVENT_TYPE` filter selects the useful integration-debugging chain consistently across both context shapes.

Put every inbound type in **one** `EVENT_TYPE` filter. Values inside a filter combine with OR, while filters on a stream combine with AND. The inbound list is `INPUT`, `STEP`, `IDENTIFIED`, `INCENTIVIZED`, `REWARD`, `SEND_REWARD`, `INTERNAL`, `MESSAGE`, and `MESSAGE_NOT_DELIVERED`. Use `REWARD` for redemption state changes.
