...
This page builds two of an integration's three baseline views: a **report-runner view** charting what the integration processed, and an **event-stream view** carrying a live feed of the events it produces. It also defines the test-event view an inbound integration adds to its `views` socket. [Integration Categories](/technical/building-custom-integrations/integration-types/integration-categories) covers what each category charts and streams.
...
curl --request POST \
  ...
  --data '{
    ...
    "component_ids": ["'"$INTEGRATION_COMPONENT_ID"'"],
    "variables": [
      { "name": "order", "type": "INTEGER", "values": { "default": 4 } },
...
### Add the Test Event View to an Inbound Integration

My Extole renders the test-event sender when the integration component has a `test-event-view-v10.0` child in its `views` socket. The integration definition is the source of the tab, like it is for configuration and event-stream views.

Create the view after the integration's business events and input rules are configured:
...
curl --request POST \
  ...
  --data '{
    ...
    "description": "Send a configured inbound event with an editable test payload.",
    "types": ["view-v10.0", "test-event-view-v10.0"],
    "installed_into_socket": "views",
    "component_ids": ["'"$INTEGRATION_COMPONENT_ID"'"],
    "variables": [
...
The view has no report runner or event stream of its own. It uses the integration's built business events, enabled input rules, and program label to prepare the payload. For a business event whose built `journeyName` is `FRIEND`, the payload includes `advocate.email` with a sanitized address distinct from the friend's `email` so the event creates a referral. Every generated payload also includes `container` with value `test`, which keeps the event out of production dashboard analytics and reports without disabling event processing or rewards. Add one test-event view for the integration, regardless of how many wire events its rules accept.
...
A bundled component declares `elements` inline; the API creates each as its own resource attached by `component_ids` — `report_runners` becomes `POST /v7/report-runners`, and `event_streams` becomes `POST /v6/event-streams`. Attach each to the **view** that displays it: the runner to `$REPORT_VIEW_COMPONENT_ID`, the stream to `$EVENT_STREAM_VIEW_COMPONENT_ID`.
...
Create the stream after the republish above. Keep its `filters` collection empty so the view records the complete processing chain across Input, STEP, reward, and delivery events.
...
Narrow the events you inspect at read time. After a synchronous test send, search the stream for the returned `event_id`, then search for descendants carrying the same `root_event_id`. This preserves events whose application context or downstream type differs from the Input while keeping each test trace precise.
...
- The stream is running with an empty `filters` collection.
