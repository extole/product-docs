...
Filter the feed to this integration's activity. `APPLICATION_TYPE` belongs on every stream. For an inbound integration, include both the partner's app type and `Event Dispatcher`.
...
So a stream that lists `API` does not capture a `POST /v6/events` submit — `API` is the consumer API, a distinct path. List `Event Dispatcher`, or leave `app_types` empty to capture every application type. A sender that sets `app_type` in the event data overrides the arrival value, so list whatever it sends instead.

Descendant events inherit the Input's `event_context`, so one value captures the Input and the events it causes.

Add an `EVENT_TYPE` filter when the integration produces one recognizable class of event, as reward fulfillment does with `{"type": "EVENT_TYPE", "event_types": ["REWARD", "SEND_REWARD"]}`. Do not add an `EVENT_TYPE` filter of `REWARD` on an inbound receive stream; that tab will never show the Input you are testing.
...
- The stream carries `APPLICATION_TYPE`. An inbound stream lists both the partner app type and `Event Dispatcher`, or leaves `app_types` empty.
