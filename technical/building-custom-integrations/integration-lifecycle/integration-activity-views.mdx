...
My Extole renders the test-event sender when the integration component has a `test-event-view-v10.0` child in its `views` socket.
...
The view builds an editable payload from the integration's business events, enabled input rules, and program label. It adds `advocate.email` with a sanitized address distinct from the friend's `email` when the selected business event has a built `journeyName` of `FRIEND`. It also adds `container` with value `test`, which keeps the event out of production dashboard analytics and reports without disabling event processing or rewards. Add one test-event view per integration.
