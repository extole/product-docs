...
curl --request POST "$EXTOLE_API_HOST/v6/event-streams/$EVENT_STREAM_ID/filters" \
  --header "Authorization: Bearer $MANAGEMENT_API_ACCESS_TOKEN" \
  --header "Content-Type: application/json" \
  --data '{"type": "APPLICATION_TYPE", "app_types": ["example", "API"]}'
```

Filter the feed to this integration's activity. `APPLICATION_TYPE` belongs on every stream. For an inbound integration, include both the partner's app type and `API`: a test submitted through `POST /v6/events` arrives with application type `API`, and a filter that lists only the partner type excludes it from the integration tab. Add an `EVENT_TYPE` filter when the integration produces one recognizable class of event, as reward fulfillment does with `{"type": "EVENT_TYPE", "event_types": ["REWARD", "SEND_REWARD"]}`. Do not add an `EVENT_TYPE` filter of `REWARD` on an inbound receive stream; that tab will never show the Input you are testing.
...
- The stream carries `APPLICATION_TYPE`. An inbound stream lists both the partner app type and `API`.
