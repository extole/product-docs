...
curl --request POST "$EXTOLE_API_HOST/v6/event-streams/$EVENT_STREAM_ID/filters" \
  --header "Authorization: Bearer $MANAGEMENT_API_ACCESS_TOKEN" \
  --header "Content-Type: application/json" \
  --data '{"type": "APPLICATION_TYPE", "app_types": ["example", "Event Dispatcher"]}'

curl --request POST "$EXTOLE_API_HOST/v6/event-streams/$EVENT_STREAM_ID/filters" \
  --header "Authorization: Bearer $MANAGEMENT_API_ACCESS_TOKEN" \
  --header "Content-Type: application/json" \
  --data '{"type": "EVENT_TYPE", "event_types": ["INPUT", "STEP", "IDENTIFIED", "INCENTIVIZED", "REWARD", "SEND_REWARD", "INTERNAL", "MESSAGE", "MESSAGE_NOT_DELIVERED"]}'
```

Filter the feed to this integration's activity. `APPLICATION_TYPE` belongs on every stream. For an inbound integration, include both the partner's app type and `Event Dispatcher`, then add one `EVENT_TYPE` filter with the inbound chain.

`app_types` matches `event_context.app_type` exactly, and the platform sets that value from the API the event arrived on:

| Arrival path | `app_type` |
| :--- | :--- |
| `POST /v6/events`, `POST /v6/async-events` | `Event Dispatcher` |
| Consumer API, off a zone URL | `API` |
| Consumer API, on a zone URL | `Web` |
| Client API | `Client API` |
| Client zone API | `Client Zone API` |
| Reward fulfillment | `Rewards` |
| Webhook dispatcher | `Webhook Dispatcher` |

So a stream that lists `API` does not capture a `POST /v6/events` submit — `API` is the consumer API, a distinct path. List `Event Dispatcher`. The API rejects an empty `app_types` list; omit the `APPLICATION_TYPE` filter entirely if you need every application type. A sender that sets `app_type` in the event data overrides the arrival value, so list whatever it sends instead.

A STEP inherits the Input's `event_context`, so `Event Dispatcher` captures both. Reward, send-reward, incentivized, identified, and message events are created with an empty `event_context`, so they pass an `APPLICATION_TYPE` filter even when they did not arrive through that application. That is why an inbound stream also carries an `EVENT_TYPE` filter: without it, the tab fills with every empty-`app_type` event in the client, and with it, any type not on the list is hidden.

Put every inbound type in **one** `EVENT_TYPE` filter. Values inside a filter combine with OR; filters on a stream combine with AND, so two `EVENT_TYPE` filters match nothing. The inbound list is `INPUT`, `STEP`, `IDENTIFIED`, `INCENTIVIZED`, `REWARD`, `SEND_REWARD`, `INTERNAL`, `MESSAGE`, and `MESSAGE_NOT_DELIVERED`. A list of `REWARD` and `SEND_REWARD` only is the reward-fulfillment filter; it hides the Input you are testing. `REDEEMED` is not on the list: redemptions emit `REWARD`.
...
- The stream carries `APPLICATION_TYPE` listing both the partner app type and `Event Dispatcher`, and one `EVENT_TYPE` filter listing `INPUT`, `STEP`, `IDENTIFIED`, `INCENTIVIZED`, `REWARD`, `SEND_REWARD`, `INTERNAL`, `MESSAGE`, and `MESSAGE_NOT_DELIVERED`.
