...
- The eight required integration display settings hold usable values, not merely present ones. Read each from `/v1/components/built` and check it against the value convention in [Create the Integration Campaign and Component Model](/technical/building-custom-integrations/integration-component-model):
