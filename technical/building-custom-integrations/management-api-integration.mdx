...
| Page | Use it for |
| :--- | :--------- |
| [Create the Integration Campaign and Component Model](/technical/building-custom-integrations/integration-component-model) | The campaign, root, and integration component; display metadata and the logo the admin renders; typed sockets and the configuration view. Every path needs this page. |
| [Add the Activity and Event Views](/technical/building-custom-integrations/integration-activity-views) | The report-runner view and event-stream view, the report runner and event stream behind them, and the republish that has to precede each attachment. Every path needs this page. |
| [Build an Outbound Library Integration](/technical/building-custom-integrations/integration-build-outbound) | Installing a maintained partner source and reshaping it, then attaching its outbound webhooks and credential. |
| [Build a Reward Fulfillment Integration](/technical/building-custom-integrations/integration-build-reward-fulfillment) | A partner that fulfills rewards: supplier type, support campaign, supplier templates, and reward webhooks. |
| [Map Inbound Partner Events](/technical/building-custom-integrations/integration-inbound-events) | Business events, input event rules, and event data for an integration that receives partner events. |
| [Normalize Inbound Events with a Prehandler](/technical/building-custom-integrations/integration-prehandlers) | Reshaping a partner's own webhook at ingest, when the partner rather than the customer decides the payload shape. |
| [Validate and Publish an Integration](/technical/building-custom-integrations/integration-validation) | Gating outbound resources, the validation checklist, and connecting the integration to a program. |
...
| Integration category | [Integration Categories](/technical/building-custom-integrations/integration-categories), which fixes the shape the rest of the build takes. |
