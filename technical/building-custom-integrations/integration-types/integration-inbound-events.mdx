...
Open the integration in **Integrations**, open its Event Stream tab, and start the stream if it is stopped. The stream must include application type `Event Dispatcher` as well as the partner type, because that is the value `POST /v6/events` arrives with. `API` is the consumer API and does not capture this submit. It must also carry one `EVENT_TYPE` filter listing `INPUT`, `STEP`, `IDENTIFIED`, `INCENTIVIZED`, `REWARD`, `SEND_REWARD`, `INTERNAL`, `MESSAGE`, and `MESSAGE_NOT_DELIVERED` — a list of `REWARD` and `SEND_REWARD` only hides the Input. [Add the Activity and Event Views](/technical/building-custom-integrations/integration-lifecycle/integration-activity-views) lists every application type and shows both filters. Search with [Event Stream Query Language](/technical/platform-integrations/extensions/event-streams-overview/event-stream-query-language):
...
| INCONCLUSIVE | The stream is stopped, filtered without `Event Dispatcher`, filtered without `INPUT` or `STEP` in its event type list, or still empty after a bounded wait. Name the stream identifier and the filter that blocked the read. |

A stream filtered to a partner application type only, to `API`, or to `REWARD` and `SEND_REWARD` only, will not show this submit. That is a misconfigured integration stream, not a reason to call the test done from **Tech Center** > **Event Streams** > **Input Events**. Use that Tech Center stream only to diagnose the missing `Event Dispatcher` filter or a missing `INPUT` or `STEP` event type, then fix the integration stream and re-run the test there.
