...
1. Search `event_id=` followed by the returned identifier to find the Input.
2. Search `root_event_id=` followed by the same identifier to find downstream events.

Open the JSON for the Input and its descendants:

- The Input's `name` is the wire event you submitted. Its `raw_data` contains the received payload, including `container: test` and, for a friend journey, `advocate.email`.
- A descendant with `type` `STEP` and the canonical `name` is the mapped business event. Its `data` contains the mapped values, and its `person_profile` should match the Input.
...
- When no campaign selects the event, `step_matching_campaign_contexts` shows each candidate. Use its `trigger_results` and `log_messages`, together with the Input's `log_messages`, to find the rule that did not match.
