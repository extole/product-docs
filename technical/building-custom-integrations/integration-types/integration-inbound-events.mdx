...
In the integration's **Test Event** view, choose a business event to generate an editable payload. The payload includes the enabled data fields configured on that business event. When a mapped data item reads a differently named partner field, the payload uses that source field name.

The generator adds two test-specific fields:

- Every payload includes `container` with value `test`, which excludes the resulting events from production dashboard analytics and reports.
- When the selected business event's built `journeyName` is `FRIEND`, the payload includes `advocate.email` with a sanitized address distinct from the friend's `email`. Extole uses it to create the referral between the two people.

Replace the generated values with representative test data before sending. A test container does not disable event processing or rewards. See [Exclude Test Data from Analytics](/technical/operational-tasks/program-testing/exclude-test-data-from-analytics).
...
- `raw_data` is the payload as received, including `container: test` and, for a friend journey, `advocate.email`.
