...
Record a verdict from what the stream shows:

| Verdict | When |
| :--- | :--- |
| PASS | The Input is present with the wire name, and a STEP with the canonical name, the same person, and the mapped values follows it on `root_event_id`. |
| FAIL | The Input is present and a STEP is missing, has the wrong canonical name, belongs to a different person, or is missing mapped values. |
