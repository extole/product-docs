...
A `2xx` means Extole accepted the event, not that it matched a business event. Watch it land before you read person events.

Open the integration in **Integrations**. If it has an Event Stream tab, start the stream and search `event_id=` followed by the identifier the send returned, using [Event Stream Query Language](/technical/platform-integrations/extensions/event-streams-overview/event-stream-query-language). A tab filtered to a partner application type, or to reward events only, will not show a submit through `POST /v6/events`. In that case open **Tech Center** > **Event Streams** and use the **Input Events** stream instead. See [Event Streams Overview](/technical/platform-integrations/extensions/event-streams-overview).

From **My Extole**, you can ask the assistant to send this test event and tell you when the Input event appears. That is the same loop, not a distinct product.
