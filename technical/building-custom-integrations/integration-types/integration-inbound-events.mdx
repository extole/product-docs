...
| INCONCLUSIVE | The stream is stopped, filtered without `Event Dispatcher`, or still empty after a bounded wait. Name the stream identifier and the filter that blocked the read. |

A stream filtered to a partner application type only, to `API`, or to reward events only, will not show this submit. That is a misconfigured integration stream, not a reason to call the test done from **Tech Center** > **Event Streams** > **Input Events**. Use that Tech Center stream only to diagnose the missing `Event Dispatcher` filter, then fix the integration stream and re-run the test there.
