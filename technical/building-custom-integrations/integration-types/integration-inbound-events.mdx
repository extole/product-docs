...
- Its `APPLICATION_TYPE` filter includes the partner type and `Event Dispatcher`, the application type assigned to `POST /v6/events`.
- One `EVENT_TYPE` filter includes `INPUT`, `STEP`, `IDENTIFIED`, `INCENTIVIZED`, `REWARD`, `SEND_REWARD`, `INTERNAL`, `MESSAGE`, and `MESSAGE_NOT_DELIVERED`.

[Add the Activity and Event Views](/technical/building-custom-integrations/integration-lifecycle/integration-activity-views) shows both filter requests. Search with [Event Stream Query Language](/technical/platform-integrations/extensions/event-streams-overview/event-stream-query-language):
...
| INCONCLUSIVE | The stream preflight is incomplete, or the configured stream remains empty after a bounded wait. Name the stream identifier and the required configuration change. |

Use the integration stream for the verdict. When its preflight requires a change, report INCONCLUSIVE and name the exact application or event type value to add. **Tech Center** > **Event Streams** > **Input Events** can confirm that Extole accepted the Input while you correct the integration stream, then re-run the test in the integration stream.
