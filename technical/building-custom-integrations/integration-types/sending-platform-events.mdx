...
An inbound integration has two halves: an integration campaign inside Extole maps arriving events onto canonical business events (see [Build a Packaged Integration with the Management API](/technical/building-custom-integrations/integration-lifecycle/management-api-integration)), and server-side code in the platform sends those events to the Events API.
...
<Warning>
A valid submission returns `200` with a JSON body carrying both `event_id` and `person_id`. Capture `event_id`; it is the correlation key for proving how the event was processed. An empty `204 No Content` means the request is not reaching the Events API — check the host.
</Warning>
...
Send one event synchronously. The response returns `event_id` and `person_id`. First find the Input in the integration's running unfiltered Event Stream by `event_id`, then find the processing chain by searching for the same value as `root_event_id`. Read the person's steps after the stream establishes the correlated result:
...
Confirm the event carries the canonical event name, not the platform's; the transaction identifier and value match the source record; person keys resolve; and resending the same source identifier produces a duplicate outcome, not a second conversion. Repeat for every event. Watch the Input and the correlated STEP on the integration Event Stream the same way [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) describes.
...
- [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events)
