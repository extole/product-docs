...
description: "Deliver a platform's lifecycle events to the Extole Events API from a server-side extension, with retries, deduplication, and verification."
---
...
Extole answers an accepted submission with a `2xx`. Network failures, `429`, and retryable `5xx` responses are worth another attempt; authentication and validation failures are not, because they fail identically on every attempt.

Expect the same event more than once. Platforms re-fire hooks and replay status history, so one lifecycle event often reaches the sender several times. Extole deduplicates on the field mapped as the unique partner event key, so a replay resolves to the same outcome rather than a second conversion.

## Personal Data in the Send Path

Whatever holds an event between the hook and delivery — a queue, a retry buffer, a log — holds customer personal data. Dropping the fields the integration does not map does not change that: the email address and customer identifier Extole resolves a person from are themselves the personal fields. Erasure requests reach undelivered events for the same reason.
...
- Redact authorization headers and request bodies in transport logs. A debug log that captures the full request re-creates, in a less protected place, the personal data the send path is careful with.
