...
Send one event synchronously and capture the returned `event_id` and `person_id`. Use `person_id` to read the person's events:
...
Confirm the event carries the canonical event name, not the platform's; the transaction identifier and value match the source record; person keys resolve; and resending the same source identifier produces a duplicate outcome, not a second conversion. Repeat for every event. [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events#how-to-test) shows how to use `event_id` to follow the complete processing chain in Event Streams.
