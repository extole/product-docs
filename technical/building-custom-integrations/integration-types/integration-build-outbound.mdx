...
description: "Forward Extole activity to a partner by installing a maintained source or building the controllers and webhooks its API requires."
---
...
An outbound integration forwards Extole program activity to a partner platform. Start from a maintained source when it includes the required controllers and webhooks. When the source contains only display metadata, add the controller-backed delivery yourself.
...
Use the integration component's `integration-v10.*` type as the product version. The root campaign wrapper has no component type.

<Warning>
An `integration-v10.*` type identifies the product version, not the source's outbound capability. A metadata-only source can carry the partner description and logo without `internal:self-managed`, child controllers, or webhooks.
</Warning>
...
Create any component type the partner's data template needs before creating the typed child. Use `POST /v1/component-types` with the partner type's name, display name, and schema, and omit `parent`. The child must carry that type; an empty `types` array satisfies no socket filter or template lookup.
...
| The installed component is the wrong source | The listing returns the maintained source plus copies installed in client accounts, in no reliable order. | Choose the Extole-owned source by owner and type. |
