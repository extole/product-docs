...
Use the integration component's `integration-v10.*` type to identify the product version. Establish outbound capability by checking the source's behavior, not its version.
...
When the source carries only display metadata, use the controller-backed path below.
...
| An integration source has a current type but no controllers or webhooks | The source is partner metadata, not a complete outbound implementation. | Build the controller-backed path. |
