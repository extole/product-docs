...
Not everything a platform sends to Extole is an event. A platform that decides who is eligible for an offer — a marketing automation or customer engagement platform holding the segment — maintains Extole audience membership directly over the API. This is inbound in direction, and it is the one inbound shape that needs a credential, because the platform authenticates to Extole rather than the other way around.

Membership arriving from a partner is not the same as an Extole audience built from Extole activity. Extole owns the second; the partner owns the first, and the integration exists to give the partner a stable audience to write to and an identity mapping to write against.

An integration that receives membership adds three things to the tree above:

- **The audience the integration works against.** Create it once, tag it to the integration, and expose its identifier as a setting on the integration component so the audience survives a rebuild.
- **The credential the partner authenticates with.** The platform sends server-side requests with a bearer token the installer creates. Name the scope the membership calls need and say where the token belongs — the platform's own secret storage, never an integration setting, client-side code, or a logged value.
- **Setup instructions naming the exact endpoints.** Pointing the installer at the audience API in general is not a deliverable; the audience identifier and the calls below are.

| Purpose | Call |
| :------ | :--- |
| Resolve a person by email | `GET /v5/persons?identity_key_value={email}` |
| Resolve a person by partner user ID | `GET /v5/persons?person_keys=partner_user_id:{external_id}` |
| Add one member | `POST /v5/persons/{person_id}/memberships` with the audience identifier in the body |
| Remove one member | `DELETE /v5/persons/{person_id}/memberships/{audience_id}` |
| Add, remove, or replace in bulk | `POST /v1/audiences/{audience_id}/operations` with a `type` of `ADD`, `REMOVE`, or `REPLACE` |
| Read current members | `GET /v1/audiences/{audience_id}/members/view/details` |

Chart this integration's activity against audience membership rather than webhook traffic, and scope the report runner to the audience the integration owns so the view answers for this partner instead of every audience in the account.
...
A request can land in more than one row, and a maintained library source answers only the rows it was built for. Installing it satisfies the partner, not the request: a capability the request named and the library does not carry is still yours to build on top of the install. Reconcile the finished integration against every row the request matched before calling it done.
