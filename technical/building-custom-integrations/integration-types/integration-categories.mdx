...
Some platforms send Extole something other than events. A marketing automation or customer engagement platform that already knows which customers are eligible for an offer can keep that group in sync as an Extole audience, adding and removing members over the API as its own segment changes.

This is still inbound, since the platform calls Extole, but it is the one inbound shape that needs a credential, because the platform has to authenticate. Keep it distinct from an audience Extole builds out of program activity: here the partner decides who belongs, and the integration gives it a stable audience to write to.

An integration that receives membership needs three things beyond the tree above:

- **An audience for the integration.** Create it once, tag it to the integration, and store its identifier as a setting on the integration component so it survives a rebuild.
- **A credential for the partner.** The platform sends server-side requests with a bearer token the installer creates and keeps in the platform's own secret storage. Never store the token in an integration setting or anywhere client-side.
- **Setup instructions with the endpoints in them.** Give the installer the audience identifier and the calls below rather than a link to the audience API.
...
| Add, remove, or replace a list | `POST /v1/audiences/{audience_id}/operations` with a `type` of `ADD`, `REMOVE`, or `REPLACE` |
| Read current members | `GET /v1/audiences/{audience_id}/members/view/details` |

[Braze](/technical/partners/marketing-automation/braze) works this way and has a full example, including the batch request body and how to poll a long-running operation.

Point the report runner at audience membership rather than webhook traffic, and scope it to this integration's own audience so the chart shows this partner instead of every audience in the account.
