...
### Scenario 3: Verify a Partner to Extole Integration

Use [Map Partner Events to Business Events](/technical/building-partner-integrations/integration-types/partner-events-map-to-business-events#how-to-test) to send a sanitized event while the integration's **Event Stream** tab is running without filters.

Find the Input by its returned `event_id`, then find descendants by the same `root_event_id`. Open their JSON to confirm the wire event produced the expected canonical STEP. The linked guide explains the fields and logs used to diagnose matching, mapping, and attribution.
