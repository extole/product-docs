...
You have mapped a partner wire event onto a canonical Extole business event and need to prove a sanitized test event is received. Send the test through `POST /v6/events` as [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) describes, then watch Event Streams.

**What to Confirm**

* The Input event appears for the returned event identifier.
* The mapped business event follows it.

**Which Stream to Open**

Open the integration's Event Stream tab when it is unfiltered enough to show an API submit. A stream filtered by partner application type, or to reward events only, will not show that submit — use **Tech Center** > **Event Streams** and the **Input Events** stream instead.
