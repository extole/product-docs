...
Open the integration's **Event Stream** tab and confirm the stream is running without filters. Send a sanitized test event as [Map Inbound Partner Events](/technical/building-custom-integrations/integration-types/integration-inbound-events) describes.

Search `event_id=` followed by the identifier returned by the send to find the Input. Then search `root_event_id=` followed by the same identifier to find the events produced from it. A successful trace contains the submitted wire name on the Input and the canonical business event on a STEP with the same person, mapped values, and attributed campaign context.

If the STEP is missing, unattributed, or has unexpected data, open the matching JSON and use `log_messages`, `step_matching_campaign_contexts`, and `trigger_results` to find the rule that did not match. Send a fresh unique event after correcting the integration.
