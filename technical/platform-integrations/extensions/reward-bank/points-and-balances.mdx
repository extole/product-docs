...
Supported Reward Bank Collectible Rewards can expire automatically on a fixed date or after a delay
from the earn date. The expiration setting applies to rewards issued after you publish the updated
configuration; it does not update rewards that participants already hold.

You can also set or change the expiration date on an individual reward in **View Reward**. When the
date passes, the reward moves to expired and is no longer available in the Redemption Center.

To expire a reward immediately, call the endpoint with an optional human-readable reason:
...
**Revoke** marks a fulfilled or sent reward as invalid when the supplier supports revocation.
`POST /v2/rewards/{reward_id}/revoke` accepts an optional reason. Extole does not automatically map
a return to the points issued for its original order; your system must identify and revoke the
reward.
