...
**Redemption Minimum Value and Redemption Maximum Value:**

Set the smallest and largest monetary amount that can be redeemed at one time. These are based on the dollar amount of the rewards and should align with the maximum reward limits in place by the gift card provider.  
Best Practice: Align with brand limits (e.g., $2,000 for Amazon gift cards)  
Setting to 0 disables specific limits.

**Redemption Ratio:**

Set how many points make one dollar of redeemable value. The default is 1, so 1 point is worth 1 dollar.

Example: Set the ratio to 10 and 10 points are worth $1

<img src="/images/extole/reward-bank-redemption-options.png" alt="Screenshot of the redemption settings showing Redemption Options, Redemption Minimum Value, Redemption Maximum Value, and Redemption Ratio" />
...
A Reward Bank balance is the sum of the person's rewards that carry the collectible reward slot. A reward without that slot does not count, even when the balance reads zero.

Compute the balance from the person's own reward list rather than from a reward list filtered by a single person id. A profile can hold several person records, because Extole creates a fresh record for a redemption center session and for some inbound partner events and then merges them into one profile. Each reward stays on the record that earned it, so a filter on one person id undercounts. A redemption consumes collectibles held anywhere in the profile.

A negative order value issues a zero-point collectible rather than a deduction. A balance never goes below zero, and no configuration subtracts points from a balance.
