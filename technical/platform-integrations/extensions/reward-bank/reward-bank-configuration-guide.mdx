...
Use `target=campaign_id:`, not `journey.campaign_id=`. Both parameters name a campaign, but only
`target=` removes the other Reward Banks from selection. `journey.campaign_id=` is a preference that
the platform can overrule.

<Warning>
  **Only the links you build can carry the campaign target.** Once the Redemption Center is open, it
  requests the customer's balance, earned collectible rewards, and redemption options from the
  `bankable_rewards` zone, and that request carries no campaign target. While more than one Reward
  Bank is live in the same account, every one of them answers it and the platform returns whichever
  it selects — so a customer can be shown another Reward Bank's balance and redemption options.

  Keep **one Reward Bank live at a time** unless Extole has confirmed your programs are unaffected.
  When you replace a Reward Bank, point the programs at the new one and disable the old one in the
  same change rather than running both.
</Warning>
