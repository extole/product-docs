...
| `code`        | No       | A custom code for the share link. Omit it and Extole generates one. See [Share Link Codes](#share-link-codes) for the rules a requested code must satisfy            |
...
Every share link ends in a code — the part after your program domain, as in `share.yourbrand.com/alex01`. Extole generates one for you, and you can also request a specific one.

### How Extole Chooses a Code

When no code is requested, Extole builds one from the person's first name, falling back to the local part of their email address. If that value is already in use, Extole appends a random number and tries again, and finally falls back to a generated code. This path always produces a share link.

### Rules for a Requested Code

| Rule | Detail |
| :--- | :--- |
| Length | 4 to 50 characters |
| Characters | Letters, digits, period, underscore, and hyphen only |
| Uniqueness | Unique across your entire account — two people cannot hold the same code, even in different programs |
| Blocked words | The code cannot contain a blocked word *anywhere inside it*, not only as a whole word. `cass.dani12` is rejected because it contains `ass` |
| Reserved words | The code cannot *begin with* a word Extole reserves for platform routes and site pages — `admin`, `login`, `help`, `blog`, `zone`, `core` and similar. A customer named Corey is refused `corey`, because it begins with `core` |

<Warning>
A requested code that breaks any of these rules is refused and **no share link is created**. Through the API you get an error such as `code_taken`, along with a list of available alternatives in `code_suggestions`. Through an extension or a campaign action there is nothing to catch the error: that person ends up with no share link, and because no share link was created, no webhook fires for them.
</Warning>

<Tip>
Codes derived from customer data collide more often than they look like they will. Two people with the same email local part at different providers — `alex@gmail.com` and `alex@yahoo.com` — produce the same code, and the second one is refused. Let Extole generate the code unless you have a specific reason to request one.
</Tip>
...
Let Extole generate the codes for a backfill. Requesting codes derived from customer data means every collision is a customer with no share link, and the collision rate climbs as coverage grows — on a list of millions, most duplicated names arrive after the first person has already taken the code. Confirm the code strategy with your Solutions Architect before the run, and ask for a count of customers who did not receive a link so you can reconcile it against your own list.
