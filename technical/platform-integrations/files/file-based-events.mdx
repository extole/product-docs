...
- A recurring inbound feed: install [File Integration](/technical/platform-integrations/extensions/file-integration) and point its job at the directory and filename pattern you will send.
...
For a File Integration job, upload the file to the directory that job's **Lands in directory** trigger watches. That path is the one you configured on the extension, not necessarily `/events`.
