...
BHN is an Extole to Partner reward fulfillment integration: Extole orders cards from BHN when a program participant earns a reward, and BHN's response moves that reward to fulfilled or failed. BHN sends no activity into Extole, so the integration carries no business events.
...
These are the maintained v10 handlers. Write them; do not leave `request` as `createRequestBuilderWithDefaults().build()` or `response_handler` null.
...
| `Funding Posted` | Fulfilled | The maintained integration closes the reward here rather than waiting for `Complete`. |
| `Cancelled`, `Declined`, `Error`, `Failure` | Retry, then fail | The maintained handler does not fail these on first sight. It returns `"RETRY"` until the schedule is exhausted, then fails the reward. |
...
For how this category is built in general terms, see [Integration Categories](/technical/building-partner-integrations/integration-types/integration-categories), [Build a Reward Fulfillment Integration](/technical/building-partner-integrations/integration-types/integration-build-reward-fulfillment) for the sequence, and [Create the Integration Campaign and Component Model](/technical/building-partner-integrations/integration-lifecycle/integration-component-model) for the campaign, component, and display metadata every path needs.
