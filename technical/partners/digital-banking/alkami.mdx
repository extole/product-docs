...
1. Go to your Program dashboard and create [a promo link](/guides/platform-overview/implementing-your-referral-program/creating-ctas/how-to-create-a-promo-link). Name it so you can track it in My Extole, for example `Alkami Embedded`.
2. Go to **Security Center** and click **+ New Access Token**. Copy the token and store it in a safe place; you will not be able to view it again.
3. Go to **Partners** > **Alkami** and install the Alkami integration. Update the promotion URL with the promo link you created above and click **Apply Changes**.

### Step 3: Set up a Standard SSO in the Alkami Platform

Complete the [prerequisites](#prerequisites) before this step.

1. In your Staging Admin Alkami account, go to **Set-Up** > **Standard SSO** > **Create Default SSO**.
2. In the Default SSO configuration, set **Name** to **ExtoleReferrals** and **Description** to **Extole Referrals and Rewards SSO**. Copy the **Bank Identifier** from that default configuration and keep it somewhere you can paste it in the next item.
3. Delete the default SSO configuration and paste the following configuration. Replace `MY_ID` with the Bank Identifier from the previous item, and `MY_TOKEN` with the access token from Step 2.
   ```text JSON
   {
...
In the Alkami admin, create a new menu item and name it Refer a Friend. You can also place banner ads on your Alkami Dashboard. Connect the menu item.
