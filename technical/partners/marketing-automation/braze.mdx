...
Braze can decide which customers are eligible for an offer and keep that group in sync as an Extole audience. This runs in the opposite direction to the rest of this page: Braze calls Extole, so it authenticates with an Extole token rather than with your Braze REST API key, and nothing here uses your Braze REST endpoint. The maintained library source does not include this path, so you add it to the integration after installing.

The calls, the batch request body, and how to poll a long-running operation are the same for any platform that works this way, and they are covered in [Receive Audience Membership from a Partner](/technical/building-custom-integrations/integration-types/integration-inbound-audience-membership). Three things are specific to Braze.

**Identify the customer.** Braze can send the email address it holds, which Extole resolves directly:
...
To use the Braze external ID instead, map it to `partner_user_id` on the integration and resolve on that key:
...
**Choose the operation type from how Braze sends the segment.** A Braze segment exported in full each time should use `REPLACE`, so the Extole audience ends up matching it exactly. Braze webhooks firing on segment entry and exit should use the single-member add and remove calls, or `ADD` and `REMOVE` batches.

**Store the Extole token in Braze.** Create it in **Security Center** in your My Extole account with permission to write audience membership, then hold it in Braze's own secret storage and reference it from the webhook or Currents configuration that calls Extole. It does not belong in an Extole integration setting.
