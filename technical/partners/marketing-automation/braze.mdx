...
Braze is mainly an outbound integration: Extole forwards program activity to Braze. It installs from a maintained library source and is then configured for your Braze account. Braze can also send audience membership into Extole — see [Send Audience Membership from Braze](#send-audience-membership-from-braze).
...
[Receive Audience Membership from a Partner](/technical/building-custom-integrations/integration-types/integration-inbound-audience-membership) covers the calls, the batch request body, and operation polling. Three things are specific to Braze:
