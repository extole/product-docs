...
Braze is mainly an Extole to Partner integration: Extole forwards program activity to Braze. It installs from a maintained source and is then configured for your Braze account. Braze can also send audience membership into Extole — see [Send Audience Membership from Braze](#send-audience-membership-from-braze).
...
For the platform build sequence — installing the maintained source, reshaping it, attaching webhooks, and verifying the result — see [Integration Categories](/technical/building-partner-integrations/integration-types/integration-categories), [Build an Extole to Partner Integration](/technical/building-partner-integrations/integration-types/integration-build-extole-to-partner), and [Create the Integration Campaign and Component Model](/technical/building-partner-integrations/integration-lifecycle/integration-component-model).
...
Braze can sync a group of eligible customers to an Extole audience. These calls run in the opposite direction to the rest of this page: Braze calls Extole and authenticates with an Extole token, not your Braze REST API key. The maintained source does not include this path; add it to the integration after installing.

[Receive Audience Membership from a Partner](/technical/building-partner-integrations/integration-types/partner-events-audience-membership) covers the calls, the batch request body, and operation polling. Three things are specific to Braze:

**Identity.** Resolve the person by the email address Braze holds:
...
Or map the Braze external ID to `partner_user_id` on the integration and resolve on that key:
...
**Operation type.** Use `REPLACE` for a Braze segment exported in full each time. Use the single-member calls, or `ADD` and `REMOVE` batches, for Braze webhooks firing on segment entry and exit.

**Token storage.** Create the server access token in **Security Center** in My Extole with permission to write audience membership. Store it in Braze's secret storage and reference it from the webhook or Currents configuration that calls Extole. Do not put it in an Extole integration setting.
