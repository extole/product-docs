...
- **Typed sockets** — the named attachment points on a component that accept only components of a declared type, such as a `views` socket that holds view components. [Create the Integration Campaign and Component Model](/technical/integration-overview/integration-component-model) describes each socket this build uses.
- **Business-event templates** — maintained components that produce a canonical Extole event such as `converted` or `shipped`, duplicated into your campaign instead of rebuilt. [Integration Categories](/technical/integration-overview/integration-categories) explains how a partner's own event names map onto them.
...
Place the platform in a category before creating anything. [Integration Categories](/technical/integration-overview/integration-categories) describes what each category contains and how to recognize it; this guide carries the API sequence that builds it. Pick the path from discovery, not from habit.

| Category | When it applies | Build path |
| :------- | :-------------- | :--------- |
| Outbound library install | The duplicatable listing already has an integration component whose name matches the partner. | Duplicate that library component with no target campaign, reshape it to the finished shape on the partner page, then attach webhooks and credentials. Follow [Build an Outbound Library Integration](/technical/integration-overview/integration-build-outbound) below. |
| Reward fulfillment | The partner supplies gift cards, prepaid cards, points, or payouts that Extole orders when a reward is earned. | Install the maintained source when one exists; otherwise build the supplier type, the support campaign of supplier templates, and the integration with its `REWARD` webhooks. Follow [Build a Reward Fulfillment Integration](/technical/integration-overview/integration-build-reward-fulfillment) below. |
...
| Page | Use it for |
| :--- | :--------- |
| [Create the Integration Campaign and Component Model](/technical/integration-overview/integration-component-model) | The campaign, root, and integration component; display metadata and the logo the admin renders; typed sockets and the configuration view. Every path needs this page. |
| [Add the Activity and Event Views](/technical/integration-overview/integration-activity-views) | The report-runner view and event-stream view, the report runner and event stream behind them, and the republish that has to precede each attachment. Every path needs this page. |
| [Build an Outbound Library Integration](/technical/integration-overview/integration-build-outbound) | Installing a maintained partner source and reshaping it, then attaching its outbound webhooks and credential. |
| [Build a Reward Fulfillment Integration](/technical/integration-overview/integration-build-reward-fulfillment) | A partner that fulfills rewards: supplier type, support campaign, supplier templates, and reward webhooks. |
| [Map Inbound Partner Events](/technical/integration-overview/integration-inbound-events) | Business events, input event rules, and event data for an integration that receives partner events. |
| [Normalize Inbound Events with a Prehandler](/technical/integration-overview/integration-prehandlers) | Reshaping a partner's own webhook at ingest, when the partner rather than the customer decides the payload shape. |
| [Validate and Publish an Integration](/technical/integration-overview/integration-validation) | Gating outbound resources, the validation checklist, and connecting the integration to a program. |
...
| Integration category | [Integration Categories](/technical/integration-overview/integration-categories), which fixes the shape the rest of the build takes. |
...
Campaigns, components, webhooks, and reward suppliers are Management API calls; event submission is a Server to Extole call. Both go to `api.extole.io`, as described in [REST APIs](/technical/platform-integrations/rest-apis/index). [Send Platform Events to Extole](/technical/integration-overview/sending-platform-events) covers the sending side in full.
