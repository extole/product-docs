...
[Create an Integration with the Management API](/technical/integration-overview/management-api-integration) — then publish or request
...
Create the `INTEGRATION` campaign, root, and model component as described in [Create the Integration Campaign](/technical/integration-overview/integration-component-model) and [Create the Component Model](/technical/integration-overview/integration-component-model). Declare the credential settings under the names the partner page uses — for BHN that is `merchantId` (`STRING`) and `clientKeyId` (`CLIENT_KEY`). A prefixed invention such as `bhnMerchantId` is a different setting, and webhook `client_key_id` expressions plus request handlers that call `context.getVariable("merchantId")` will read null. Then add the supplier socket, filtered to the type created above:
