...
- Confirmation that the duplicatable listing already has a maintained integration component whose name matches the partner. If it does not, this is not an outbound library install — use [Create the Integration Campaign and Component Model](/technical/integration-overview/integration-component-model) instead.
