...
The ratio at which accumulated reward value converts into redeemable value. The default is 1:1, so one point is one dollar; a ten-point rate means a participant earns one dollar for every ten points.

*The redemption rate sets both what a program costs and how the offer reads to a participant, so it is worth choosing deliberately before launch.*
