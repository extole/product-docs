...
<img src="/images/extole/3825a72f785d152c6822f5276ed792821efaee8b4b1ee942cc84e18ef5315f85.png" alt="Screenshot showing You can see detailed metrics for how each of your campaigns performs at each stage of the referral funnel in your Extole dashboard, then test and iterate so..." width="80%" />
...
[//]: # "What is a reward bank?"

The extension that holds a participant's accumulated reward value and lets them redeem it when they choose. Rather than issuing a small reward at the moment of every action, a Reward Bank lets value build until it is worth spending, which spreads reward cost over time and gives participants a reason to come back.

*An account can run more than one Reward Bank, so distinct programs or reward types can each have their own.*

[//]: ___

### Way to Earn

[//]: # "What is a way to earn?"

A business event paired with a reward rule, configured inside a **Reward for Action** program. Each way to earn names the action that earns points — a purchase, a review, an app download, a birthday — and the Collectible Reward it issues. There is no set limit on how many a program can have.

*Start with the ways to earn that use events already flowing into Extole, then add more.*

[//]: ___

### Collectible Reward

[//]: # "What is a collectible reward?"

Reward value earned by completing an eligible action, held in the participant's Reward Bank until they redeem it. Collectible Rewards are what a loyalty program issues for each way to earn — a set number of points for a review or an app download, or a percentage of the transaction for a purchase.

*Create one Collectible Reward per way to earn, and name it so its value is obvious in a list.*
...
A denomination for reward value, alongside currencies and percentages. Points are not a separate system: a point-denominated reward moves through the same states, rules, and reporting as any other reward, and carries a face value type of `POINTS`.

*Loyalty Points is one of the types available when creating a custom reward, alongside Account Credit.*
...
The interface where a participant sees the rewards they have earned, the options available to them, and their redemption history. It can be embedded in a page you already own, such as an account page, or used as a standalone branded page linked from email.

*Participants must reach the Redemption Center as a verified consumer, so a balance is never shown to the wrong person.*
...
The configurable set of rewards a participant can convert their accumulated value into — gift cards, coupons, account credits, or a custom reward you fulfill yourself.

*Redemption Options are what decide whether a balance feels worth building, so they matter as much as the rate.*

[//]: ___

### Redemption Ratio

[//]: # "What is a redemption ratio?"

How many points make one dollar of redeemable value, set on the Reward Bank extension. The default is `1`, so one point is worth one dollar. Set it to `10` and ten points are worth one dollar.

*The Redemption Ratio sets both what a program costs and how the offer reads to a participant, so it is worth choosing deliberately before launch.*
