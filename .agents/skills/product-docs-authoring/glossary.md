...
Preferred terms for Extole customer-facing documentation (`extole/product-docs`, on Mintlify). This is the agent-actionable distillation used by the **product-docs-authoring** skill and the **product-docs-style** rule.

**Status: proposed — for docs-team ratification.** Counts come from ripgrep across the 461 pages of the ReadMe corpus these pages were migrated from (2026-07); they describe established Extole usage, not this repo's file layout. Where the corpus is genuinely split, the term is listed under **Open decisions** below rather than given a verdict — those need a human call before an agent enforces them. The canonical human-owned sources are the **Extole Style Guide** and **Extole Content Strategy Outline** (Google Docs); this file defers to them and should be updated when they change.
...
5. **Callout syntax.** Largely settled by the platform: Mintlify ships `<Info>` / `<Tip>` / `<Warning>` / `<Danger>` / `<Note>`, and the migration already converted the ReadMe emoji-blockquote callouts to them across 76 pages. Use the components; the remaining call is editorial — which severity maps to which situation.
6. **Headings vs bold-line pseudo-headings.** Some pages use bold lines instead of `##`, which leaves them out of Mintlify's on-page TOC. **Proposal:** always use `##`.
