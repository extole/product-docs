...
The agent-actionable subset of Extole's documentation style, for `extole/product-docs` → docs.extole.com. Pairs with [`glossary.md`](glossary.md) (terminology) and the [`product-docs-authoring`](SKILL.md) skill.
...
- **Overview first.** Open most pages with `# Overview` — what it is and why it matters (262/461 pages do). Often lead with a one-sentence definition that mirrors the `excerpt`.
- **Real headings.** Use `##`/`###`, not bold-line pseudo-headings, so ReadMe generates the table of contents. ⚑ Some newer pages use bold lines — don't copy that.
...
- **Emphasis / asides:** bold `**Note**:` inline, or a `>` blockquote. There is no callout/admonition syntax in the corpus yet (see glossary Open decision #5).
...
`title` (required, quoted) and `excerpt` (strongly recommended) are the common fields. Others seen in the corpus: `slug` (ReadMe slug override), `hidden`, `deprecated`, `metadata` (SEO), `next`, `parentDoc`, `category`. Add the page's slug to the category `_order.yaml` so it appears in nav.
