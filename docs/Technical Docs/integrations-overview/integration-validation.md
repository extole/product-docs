...
- Every event carries an identity the account can reach, which in practice means the email address wherever the partner sends one. Where a prehandler flattens the payload, read its actions and confirm the email is among the keys it emits: a partner nests the address away from the transaction fields, so an integration that captures the amount and the order and not the address is the normal outcome rather than an unusual one, and it raises no error. A partner that sends no address at all leaves this unmet, and that is an outstanding item to report rather than a box to tick. [Normalize Inbound Events with a Prehandler](doc:integration-prehandlers) carries the same contract.
