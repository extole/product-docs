...
A data component can only read a key the prehandler left behind, so the reshape decides what identity the platform ever sees. One contract governs it, and [Validate the Integration](doc:integration-validation) checks the same one: every event carries an identity the account can reach. Flatten the full set that [Map Inbound Partner Events](doc:integration-inbound-events) requires — the transaction identifier, the partner's customer identifier, and the email address — on every event, including the lifecycle ones that arrive later.

An email address is what satisfies that contract in practice, so emit it wherever the partner sends one. A partner that genuinely sends none leaves the contract unmet, and that belongs in the build report as an outstanding item: the integration will recognise a returning customer and will not be able to message or reward one.
