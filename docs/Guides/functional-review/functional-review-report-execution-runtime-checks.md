...
3. Re-read the specific report section and verify section-level conditions:
   ...
   - submitted `report_type` matches the documented type for that queue item (Triggered Steps must be `INPUT_EVENTS_WITH_TRIGGERED_STEPS`, never Count’s `t1owor6ia18bia3ur7zg`; Conversion Audit `step_names` must pass the V8 / V10 enabled-reward test in [Conversion & Reward Validation](doc:functional-review-conversion-reward-validation) — V10: enabled `earn-reward` or `qualify` only, not bare `incentivize`; V8: enabled purchase `incentivize` only)
...
- Conversion Audit attempted with `step_names` that pass the V8 / V10 test in [Conversion & Reward Validation](doc:functional-review-conversion-reward-validation) (V10: enabled `earn-reward` or `qualify` only — bare `incentivize` is not enough; V8: enabled purchase `incentivize` only; disabled qualifying reward actions omitted from `step_names` and listed in the review output)
...
- Conversion Audit submitted with over-broad `step_names` (full business-event funnel dump; V10 steps with only bare `incentivize` and no enabled `earn-reward` / `qualify`; V8 steps with only non-purchase `incentivize`; `canceled` / `shared` / other steps that fail the V8 / V10 test; steps whose only qualifying reward actions are disabled)
