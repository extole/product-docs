...
The entity names below are the platform records a report reads. A **StepRecord** is one step in the referral lifecycle recorded on a person's profile — a share, a click, a conversion, a reward earned; an **InputRecord** is one raw inbound event from a client integration, before it is processed into a step. Full field lists for every entity are in [Entities and Context](doc:entities-and-context-available-in-extole-s-configurable-reporting-system).
