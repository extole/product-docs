...
<Info>
  **New to Program includes New to Client.** The visit types nest rather than divide your data. A friend whose referred purchase is also their first-ever purchase with your brand is new to the client *and* new to the program, so a **New to Program** filter counts them too. **New to Client** is the narrower of the two views.

  When a report breaks events out by visit type, each event still carries a single label, and **New to Client** wins wherever both apply. The events labelled *New to Program* are therefore only part of what a **New to Program** filter returns — to reconcile a report with a dashboard tile, add the *New to Client* events to them.
</Info>
...
**Advocate events** such as Promotion Clicks, Shares, or Share Clicks will use the **All visit type**, as it’s an event that you would want to track every time it’s done.
 ...
 ![Screenshot showing Advocate events such as Promotion Clicks, Shares, or Share Clicks will use the All visit type, as it’s an event that you would want to track every time it’s...](/images/extole/8791804f9a8bc53f796d8235efb9edad78ce05eb16428ed09bbe014ebb190bdc.png)
