...
description: "Set up program targeting using Extole’s upgraded Flow Builder."
---
...
- **Country List** – The countries the rule checks traffic against

  - Format: ISO country codes (e.g., `US`, `CA`)

- **Use Country List as a whitelist** – On, only the listed countries pass. Off, the listed countries are blocked.

<Note>
  Some events carry no visitor IP address — a business event created by a file upload or a server-side API call, for example. For those events the country check reads every location Extole has recorded for the participant instead. When the Country List is used as a whitelist, every one of those locations must be in the list.

  A participant who visited once from outside the list therefore fails this rule on every later file-driven event, even after visiting again from a listed country. When you test with an uploaded file, connect from a listed country before the first page view, or use a participant who has only ever been seen from one.
</Note>
