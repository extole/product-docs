...
A reward has two names, and renaming it is a change to your campaign, not just to a label.

- **The name at the top of the reward** is the one you see and edit in the Flow Builder. Editing it also sets the reward's internal name: Extole lowercases what you typed and replaces spaces with underscores, so "Advocate Gift Card" becomes `advocate_gift_card`. The reward's **Journey Name** follows that, and the reward's journey and step events are named after it in turn.
- **Reward Name** is the identifier stamped on each reward issued. Reward rules such as **Has Not Been Rewarded For Relationship** match on it, and the reward's own events are named after it — a reward named `converted_advocate` records `converted_advocate_reward_earned` and `converted_advocate_reward_fulfilled`. Metrics that count those events reference them by name. It is an Expert setting, so turn on **Show Expert** in the reward's settings filter to see it.

**Reward Name** only stays put if it has been given a fixed value. Left on its default, it follows the name at the top like everything else — so check it under **Show Expert** before you rename, and again afterwards.

<Warning>
Rename freely while a campaign is still in draft. On a live campaign, treat a rename as a behavior change: rewards and events recorded before the rename keep the names they were recorded under. A rule that checks whether a person has already been rewarded stops seeing them, and any metric whose expression references the old event name stops counting until you update the expression.
</Warning>
...
- [How to Configure Reward Expiration](/guides/rewards-management/setting-up-your-rewards/how-to-configure-reward-expiration)
