...
description: "Understand the trigger rules available when building campaigns on Flow Builder"
---
...
> **Trigger rules** are a type of **[Business Event Rule](/guides/flow-campaigns/how-to-set-up-business-event-rules)** that let you control precisely which incoming events from your systems create or process business events inside Extole. Use them to decide *when* Extole should record a business event based on event payloads, a person’s past behavior, audience membership, or technical context. If an input event **fails trigger rules**, Extole **does not** create the business event at all.​
...
> Some of these rules can also be implemented as **[Reward Rules](/guides/flow-campaigns/how-to-set-up-reward-rules)**, **[Quality Rules](/guides/flow-campaigns/default-business-event-quality-rules)**, or **[Targeting Scenarios](/guides/flow-campaigns/how-to-target-a-program#h_2b6a567239)** and are indicated as such below.
...
- The comparison ignores case, so an event sent as `Free_Trial_Started` matches a step listening for `free_trial_started`. If your systems send the same event in more than one casing, you don't need to list each spelling.
...
This rule reads the profile rather than the event, so the parameter has to be on the profile
before the rule runs. To put it there from an event you send, prefix the event data key with
`me.public.` or `me.private.` — see
