...
<Warning>
  Changing a rule updates a draft of the campaign. The new setting is used only after the campaign is published. If the campaign has not launched yet, publishing happens when you take it live; if it is already live, contact your CSM to publish the change.

  Rules apply to events as they arrive, so publishing a change affects new events only. Extole does not re-score events that were already processed, and an event held by a rule you have since turned off stays held.
</Warning>
...
| --- | --- | --- |
| Bot Multi-Click Prevention | A second click or sign up by the same person within 30 seconds of one that already scored high quality is invalid. Someone who opens two different share links in quick succession trips this rule, even when nothing about the traffic is automated. | Internally configured. The 30 second window is fixed; the rule can only be turned on or off. |
