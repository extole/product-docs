...
### Attributing the Event to a Program

An uploaded event that carries no program label is still processed, but it isn't attributed to a campaign. On the **User Support** page it appears with no program, no campaign, and no journey, at **Quality = NONE**, and no reward rule evaluates it.

To attribute the event, add a `required_labels` column and set it to your program label. Find the program label on the **Programs** page.

| Column | Example | Description |
|---|---|---|
| `event_name` | `outcome` | The event name your campaign listens for. |
| `email` | `jane@example.com` | The participant's email address. |
| `required_labels` | `offer` | The program label of the program the event belongs to. |

<Warning>
`program_label` is not a column Extole reads. Attribution comes from `required_labels`, `labels`, or `campaign_id`. A header with any other name is treated as ordinary event data, so the upload succeeds, no error is reported, and the event arrives unattributed.
</Warning>

For a worked example, see [How to Set Up a Reward for Action Offer Program](/guides/programs-and-campaigns/how-to-set-up-a-reward-for-action-offer-program).
