...
- Emails: The campaign’s advocate emails stop sending. See [What Happens When a Campaign Is Paused](/guides/programs-and-campaigns/managing-campaigns/what-happens-when-a-campaign-is-paused).
...
<Warning>
  **Publish the campaign before you rely on a scheduled go-live.** A campaign that has never been
  published stays in Not Launched when its start date arrives. The start date is accepted and shown
  in the campaign list, but it has no effect, and nothing tells you it did not work.

  This matters most in the hand-off above: if the campaign you scheduled to go live was the only one
  due to take over, the program is left with no live campaign at all. Advocates stop seeing calls to
  action, new referrals stop, and share links already out in the world land on nothing.

  On the Programs page, a campaign that still needs publishing shows **Unpublished changes** next to
  its state. Publish it, then confirm after the scheduled time that it shows **LIVE**.
</Warning>
